...
def Boxing(NEPvm,Pvm,N_Pvm,CPU_kernel,CPU_memory,condition):
    ...
    VMs=First_OP(VMs,CPU_kernel,CPU_memory,condition)
...
def First_OP(VMs,CPU_kernel,CPU_memory,condition):
    ...
    if((N-0.5)>int(N)):
        N=int(N)+1
        VMs=First_Add(VMs,T_k,T_m,CPU_kernel,CPU_memory,condition,N)
    else:
        N=int(N)
        VMs=First_Delete(VMs,T_k,T_m,CPU_kernel,CPU_memory,condition,N)
    '''
    N=int(N)
    VMs=First_Delete(VMs,T_k,T_m,CPU_kernel,CPU_memory,condition,N)
    return VMs
    
def First_Delete(VMs,T_k,T_m,CPU_kernel,CPU_memory,condition,N):
    if(condition==0):
        index=[]
        for i in range(len(VMs)):
