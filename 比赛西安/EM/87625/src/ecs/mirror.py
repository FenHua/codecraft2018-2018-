...
def Smirror(lenD,N_Pvm,S_Data,Predict_time,V):
    K=4
    NEPvm=[0 for i in range(N_Pvm)]
    for i in range(N_Pvm):
        Matrix=TSplit(lenD,N_Pvm,S_Data[i],Predict_time)
        NEPvm[i]=int(Estimation(Matrix,Predict_time,K)*1.2)+V
    return NEPvm

def Commirror(lenD,N_Pvm,S_Data,Predict_time,V):
    NEPvm=[0 for i in range(N_Pvm)]
    for i in range(N_Pvm):
        ...
        NEPvm[i]=int(0.5*Value1+V*Value2)
    return NEPvm
